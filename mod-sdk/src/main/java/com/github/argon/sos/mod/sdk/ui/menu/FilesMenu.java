...
public class FilesMenu extends Section {
    ...
    public FilesMenu(
        ...
        for (Path path : paths) {
            ...
            for (int i = 0; i < segments.length; i++) {
                ...
                if (currentPathNode != null && currentPathNode.has(nodeKey)) {
    ...
    /**
     * Collapses or expands a {@link FileNode}.
     *
     * @param fileNode to expand or collapse
     * @param collapse whether it shall be collapsed or expanded
     */
    public void collapse(FileNode fileNode, boolean collapse) {
        ...
        if (collapse) {
            for (TreeNode<Path> pathTreeNode : toCollapse) {
                ...
                FileNode rowValue = row.getValue();
                if (rowValue != null) {
                    rowValue.setCollapsed(false);
                }
    ...
    /**
     * Describes a node, which can be a file or folder in the file tree.
     */
    @Getter
    @Builder
