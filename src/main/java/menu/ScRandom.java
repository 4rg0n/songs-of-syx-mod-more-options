...
class ScRandom extends GuiSection implements SC{
	...
	ScRandom(Menu menu){
		...
		CLICKABLE goButton = new Screener.ScreenButton(¤¤go) {
			@Override
				...
				menu.start(new Constructor() {
					...
					public CORE_STATE getState() {
						...
						for (String k : BOOSTS.keys()) {
							double v = BOOSTS.get(k);
							if (v > 0) {
								GAME.achieve(false);
								FACTIONS.player().bonusesCustom.add(k, v, false);
							}
		...
		CLICKABLE moreOptions = new Screener.ScreenButton(MoreOptionsScript.MOD_INFO.name) {
			@Override
			protected void clickA() {
				menu.switchScreen(UiFactory.moreOptionsFullsWindow());
			}
		};

		screen.addButt(moreOptions);
		screen.addButt(goButton);
		...
		options.addRelBody(16, DIR.S, new RENDEROBJ.RenderImp(Screener.inner.width()-4, 64) {
	...
	final class Boosts extends GuiSection {
		...
		Boosts(int height, GETTER_IMP<CharSequence> hText){
			...
			addLine(rows, boos, values, hText);
			...
			addRelBody(4, DIR.N, new HOVERABLE.Sprite(UI.FONT().H2.getText(¤¤Difficulty), COLORS.label));
		}
		
		private void addLine(LinkedList<RENDEROBJ> rows, Boostable[] boos, double[] values, GETTER_IMP<CharSequence> hText) {
