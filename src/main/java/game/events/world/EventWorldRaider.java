...
public class EventWorldRaider extends EventResource{
	...
	private WArmy spawn(int men, double training, double gear) {
		...
		while (AD.men(null).get(a) < men && a.divs().canAdd()) {


			int am = CLAMP.i((men-AD.men(null).get(a)), 0, Config.BATTLE.MEN_PER_DIVISION);
			if (am < 1)
				break;

			WDivRegional d = WORLD.ARMIES().regional().create(FACTIONS.player().race(), (double)am/Config.BATTLE.MEN_PER_DIVISION, a);
			d.randomize(gear, (int)(training*15));
			d.menSet(d.menTarget());
		}
	...
	public static class Chance implements DOUBLE {
		...
		private double getChance() {
			...
			chance *= armyValue;
			return chance /= BOOSTABLES.CIVICS().RAIDING.get(RACES.clP(null, null));
		}
		...
		protected RENDEROBJ makeSection() {
			...
			section.addRelBody(48, DIR.S, new GButt.ButtPanel(¤¤pay) {
				...
				protected void clickA() {
					if (!timer.hasExpired()) {
						FACTIONS.player().credits().inc(-timer.credits, FCredits.CTYPE.MISC);
						SETT.ROOMS().STOCKPILE.removeFromEverywhere(0.1, RBIT.ALL, FResources.RTYPE.SPOILS);
